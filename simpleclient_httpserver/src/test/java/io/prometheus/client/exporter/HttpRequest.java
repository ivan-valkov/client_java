...
public class HttpRequest {
    ...
    /**
     * Method to execute an HTTP request and decompresses it
     *
     * @return HttpResponse
     * @throws IOException
     */
    public HttpResponse executeAndDecompress() throws IOException {
        if (configuration.url.toLowerCase().startsWith("https://") && (configuration.trustManagers != null)) {
            try {
                SSLContext sslContext = SSLContext.getInstance("SSL");
                sslContext.init(null, configuration.trustManagers, new java.security.SecureRandom());
        ...
        String body = this.decompress(stream);

        return new HttpResponse(
                ((HttpURLConnection) urlConnection).getResponseCode(),
                urlConnection.getHeaderFields(),
                urlConnection.getContentLength(), body);
    }

    private String decompress(InputStream stream) throws IOException {
